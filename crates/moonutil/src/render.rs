...
use crate::{common::{line_col_to_byte_idx, PatchJSON, MOON_DOC_TEST_POSTFIX}, error_code_docs::get_error_code_doc};
...
impl MooncDiagnostic {
    pub fn render(content: &str, use_fancy: bool, check_patch_file: Option<PathBuf>, explain: bool) {
        ...
        let mut report_builder = ariadne::Report::build(kind, (&display_filename, start_offset..end_offset))
            .with_label(
        ...
        if explain {
            ...
            report_builder = report_builder.with_message(format!("[{}]\n{}", diagnostic.error_code, error_code_doc).fg(color));
        }
        else {
            report_builder = report_builder.with_message(format!("[{}]", diagnostic.error_code).fg(color));
        }
